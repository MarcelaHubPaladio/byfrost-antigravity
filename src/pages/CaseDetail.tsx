...
export default function CaseDetail() {
  ...
  const waMsgsQ = useQuery({
    queryKey: ["wa_messages_case", activeTenantId, id],
    enabled: Boolean(activeTenantId && id),
    queryFn: async () => {
      const { data, error } = await supabase
        .from("wa_messages")
        .select("id,direction,type,from_phone,to_phone,body_text,media_url,occurred_at")
        .eq("tenant_id", activeTenantId!)
        .eq("case_id", id!)
        .order("occurred_at", { ascending: false })
        .limit(100);
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-[22px] border border-slate-200 bg-white p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                    <MessageSquareText className="h-4 w-4 text-slate-500" /> Conversa (WhatsApp)
                  </div>
                  <div className="text-xs text-slate-500">{waMsgsQ.data?.length ?? 0}</div>
                </div>

                <div className="mt-3 space-y-2">
                  {(waMsgsQ.data ?? []).slice(0, 18).map((m: any) => {
                    const summary =
                      m.type === "image"
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="text-xs font-semibold text-slate-900 truncate">{summary}</div>
                          ...
                          <div className="flex flex-col items-end gap-1">
                            ...
                            <div className="text-[11px] text-slate-500">{new Date(m.occurred_at).toLocaleString()}</div>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {(waMsgsQ.data ?? []).length > 18 && (
                    <div className="flex items-center justify-center gap-2 text-xs text-slate-600">
                      <ArrowDownUp className="h-4 w-4 text-slate-400" /> mostrando as 18 mais recentes
                    </div>
                  )}

                  {(waMsgsQ.data ?? []).length === 0 && (
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-white/60 p-4 text-xs text-slate-500">
