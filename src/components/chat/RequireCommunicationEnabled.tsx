...
export function RequireCommunicationEnabled({ children }: { children: ReactNode }) {
  ...
  const enabled = isCommunicationEnabled(activeTenant.modules_json);
  if (enabled) return <>{children}</>;

  return (
    <AccessRedirect
      title="Comunicação desabilitada"
      description="O módulo de Comunicação não está habilitado para este tenant. Peça ao admin para ativar em Admin → Módulos."
      to="/app"
      toLabel="Voltar"
      details={[
        { label: "tenant", value: activeTenantId },
        { label: "módulo", value: "communication" },
      ]}
    />
  );
}
