...
export function WhatsAppConversation({
  ...
  // Prefer customer_accounts phone, then entity phone, and fallback to case metadata
  const entityPhone = caseQ.data?.customer_accounts?.phone_e164 || entityQ.data || (caseQ.data?.meta_json?.monitoring?.whatsapp_number as string)?.replace(/\D/g, "") || null;
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex flex-col border-b border-slate-200 bg-white/80 backdrop-blur">
        ...
        <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 dark:border-slate-850">
          <div className="min-w-0 flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            {tokensQ.data && tokensQ.data.totalTokens > 0 && (
              <div className="flex-shrink-0 flex items-center gap-1.5 rounded-full bg-emerald-50 border border-emerald-200/80 px-2.5 py-1 text-[10px] font-semibold text-emerald-800 dark:bg-emerald-950/40 dark:border-emerald-900/60 dark:text-emerald-400">
          ...
          <div className="flex items-center gap-1.5 self-end sm:self-auto">
            ...
            >
              <MessagesSquare className="mr-1.5 h-3.5 w-3.5" /> Mensagens
            </Button>
            <Button
              type="button"
            ...
            >
              <Users className="mr-1.5 h-3.5 w-3.5" /> Participantes
            </Button>
          </div>
        </div>

        {/* Row 2: Action Buttons */}
        <div className="flex flex-wrap items-center gap-2 px-4 py-2 bg-slate-50/50 dark:bg-slate-900/10">
          ...
          <Button
            ...
            size="sm"
            className={cn(
              "h-7 rounded-lg px-2.5 gap-1.5 text-[11px] transition-all border",
              beeiaIsPaused
          ...
          >
            {beeiaIsPaused ? (
              <><BotOff className="h-3 w-3" />{togglingBeeIA ? "Retomando…" : "IA pausada"}</>
            ) : (
              <><Bot className="h-3 w-3" />{togglingBeeIA ? "Pausando…" : "Pausar IA"}</>
            )}
          </Button>
          ...
          {!beeiaIsPaused && (
            <Button
              ...
              variant="secondary"
              size="sm"
              className="h-7 rounded-lg px-2.5 gap-1.5 text-[11px] border-violet-200 bg-violet-50 text-violet-700 hover:bg-violet-100"
              onClick={retakeAI}
              disabled={retakingAI}
          ...
          {!beeiaIsPaused && (
            <div className="flex items-center gap-1 border border-indigo-200 bg-indigo-50 rounded-lg h-7 px-1">
              <select
                value={hoursLimit}
                onChange={(e) => setHoursLimit(Number(e.target.value))}
                className="bg-transparent text-indigo-750 text-[10px] outline-none cursor-pointer font-semibold pl-1"
              ...
              <Button
                ...
                variant="ghost"
                size="sm"
                className="h-5 rounded-md px-1.5 gap-1 text-[10px] text-indigo-750 hover:bg-indigo-100 hover:text-indigo-800"
          ...
          <div ref={scrollerRef} className="min-h-0 flex-1 overflow-y-auto px-3 py-4 sm:px-4">
            ...
            <div className="space-y-3">
              {(waMsgsQ.data ?? []).map((m) => {
                ...
                const isExternalOutbound = !effectiveInbound && m.payload_json?.fromApi === false;
                ...
                return (
                  ...
                  >
                    ...
                    <div className={cn("max-w-[78%]", effectiveInbound ? "mr-auto" : "ml-auto")}>
                      ...
                      >
                        {isSystemNote ? "Sistema (Interno)" : (effectiveInbound ? `${inboundLabel}${m.from_phone ? ` • ${m.from_phone}` : ""}` : isExternalOutbound ? "Celular/Web" : "Painel")} • {fmtTime(m.occurred_at)}
                      </div>
                    </div>
                  </div>
                );
