...
import { Upload, X, FileText, Loader2, User, MapPin, UserPlus, ClipboardList, Check, ChevronsUpDown, Calendar as CalendarIcon } from "lucide-react";
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const fields = [
        ...
        { case_id: orderDate && caseId, key: "sale_date_text", value_text: orderDate ? format(orderDate, "dd/MM/yyyy") : "", source: "admin", confidence: 1, last_updated_by: "panel" },
      ...
      <DialogContent className="sm:max-w-[600px] rounded-[32px] overflow-hidden p-0 border-none shadow-2xl">
        ...
        <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 flex flex-col">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <CalendarIcon className="w-3 h-3" /> Data do Pedido
              </Label>
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn(
                      "h-12 w-full justify-start text-left font-normal rounded-2xl border-slate-200",
                      !orderDate && "text-slate-500"
                    )}
                  >
                    <CalendarIcon className="mr-2 h-4 w-4 opacity-50" />
                    {orderDate ? format(orderDate, "dd/MM/yyyy", { locale: ptBR }) : <span>Selecione uma data</span>}
                  </Button>
                </PopoverTrigger>
