-- Migration: RPC for Intelligent WhatsApp Audit Ingestion
-- Author: Antigravity
-- Date: 2026-02-19
...
create or replace function public.ingest_whatsapp_audit_message(
    ...
    p_from_phone text,
    p_to_phone text,
    p_participant_phone text, -- The normalized phone of the contact (even in groups)
    p_group_id text, -- ID do grupo se houver
    p_body_text text,
    p_media_url text,
...
declare
    ...
    v_lock_key bigint;
begin
    -- 1. Advisory Lock based on participant + group to avoid race conditions on conversation upsert
    v_lock_key := ('x' || substr(md5(coalesce(p_group_id, p_participant_phone)), 1, 16))::bit(64)::bigint;
    ...
    values (
        ...
        left(p_body_text, 200), p_occurred_at, 1, now()
    )
    ...
    -- If no case exists for Audit, and it's an inbound message, create one.
    ...
    return query select true, v_conv_id, v_msg_id, 'ingested'::text;
