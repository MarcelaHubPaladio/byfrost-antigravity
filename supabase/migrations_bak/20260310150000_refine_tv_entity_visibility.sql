-- Refine tv_corporativa_get_timeline_by_code to strictly respect entity plan visibility
-- ensures only active videos from active entity plans are shown on the TV.
...
BEGIN
    ...
    -- 3. Gather medias
    -- Filters:
    -- - ep.is_active = true (the main toggle for entity visibility on TV)
    -- - ep.deleted_at IS NULL
    -- - ce.deleted_at IS NULL (entity exists)
    -- - p.deleted_at IS NULL (plan exists)
    -- - m.status = 'active' (media itself is approved/active)
