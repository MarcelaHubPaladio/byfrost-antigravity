...
begin
    ...
    if v_case_id is null then
        ...
        if v_case_id is not null then
            ...
            insert into public.timeline_events (tenant_id, case_id, event_type, message, occurred_at)
            values (p_tenant_id, v_case_id, 'lead_reactivated', 'Lead reativado por nova mensagem.', now());
            v_reactivated := true;
        end if;
    end if;
    ...
    if v_case_id is null then
        ...
        if (p_type = 'text' and (p_options->>'create_case_on_text')::boolean = false) or
           ...
           -- Skip creation
            ...
            insert into public.timeline_events (tenant_id, case_id, event_type, message, occurred_at)
            values (p_tenant_id, v_case_id, 'case_opened', 'Case aberto via WhatsApp (' || p_type || ').', now());
