...
import { buildBeeIASystemPrompt } from "../_shared/beeiaPromptBuilder.ts";
...
`;
  ...
  if (csGroup.prompt_context) {
    systemPrompt += `\nInstruções Específicas do Grupo:\n${csGroup.prompt_context}\n`;
  }
  ...
  const replyText = aiRes.text.trim();

  // 6. Send message via Z-API/Evolution
  const sendRes = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/integrations-zapi-send`, {
    method: "POST",
    headers: {
  ...
  try {
    ...
    for (const job of locked) {
      try {
        ...
        if (job.type === "BEEIA_PROCESS_MESSAGE" || job.type === "BEEIA_PROCESS_META_MESSAGE") {
          ...
          if (messageId) {
            const { data: tempMsg } = await supabase
              ...
              .eq("id", messageId)
              .maybeSingle();
            if (tempMsg?.body_text) inboundHistoryMsgs.push(tempMsg.body_text);
          }
          if (msgs && msgs.length > 0) {
            const history = msgs
              .filter((m: any) => m.direction === "inbound" && m.body_text)
              .map((m: any) => m.body_text!);
          ...
          if (discordMatch && discordMatch[1]) {
            ...
            if (webhookUrl) {
              ...
              }).catch(err => console.error("[BEEIA] Failed to send Discord notification", err));
            }
          }
        ...
        if (job.type === "BEEIA_PROCESS_CS_MESSAGE") {
          await processBeeIaCSMessageJob({ supabase, job });
          await supabase.from("job_queue").update({ status: "done" }).eq("id", job.id);
          results.push({ id: job.id, ok: true });
          continue;
        }
