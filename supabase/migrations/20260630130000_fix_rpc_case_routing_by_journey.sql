-- Migration: Update process_zapi_inbound_message to scope case searches by journey
-- Description: Restricts case resolution so messages are correctly routed to cases within the target journey, preventing crosstalk between different journeys.
...
begin
    ...
    if p_sender_is_vendor then
        ...
        if v_customer_id is null then
            insert into public.customer_accounts (tenant_id, phone_e164, name, assigned_user_id, meta_json)
            values (p_tenant_id, p_from_phone, coalesce(nullif(trim(p_contact_label), ''), 'Novo Contato'), p_assigned_user_id, jsonb_build_object('source', 'whatsapp'))
            returning id into v_customer_id;
        end if;
    end if;

    -- 4. CASE RESOLUTION
    -- A. Search Active (scoped by journey_id to avoid crosstalk)
    ...
    where tenant_id = p_tenant_id
      ...
      and (
          ...
          or assigned_user_id = p_assigned_user_id 
    ...
    -- B. Search Deleted (Reactivation - scoped by journey_id to avoid crosstalk)
        ...
        if (p_type = 'text' and (p_options->>'create_case_on_text')::boolean = false) or
           ...
           -- Skip creation
            ...
            insert into public.cases (
                tenant_id, journey_id, customer_id, assigned_vendor_id, assigned_user_id,
                status, state, case_type, title, 
                created_by_channel, meta_json
            ) values (
                p_tenant_id, v_journey_id, v_customer_id, v_vendor_id, p_assigned_user_id,
                'open', v_initial_state, 'order', coalesce(nullif(trim(p_contact_label), ''), 'Novo Contato'),
                'whatsapp',
                jsonb_build_object(
