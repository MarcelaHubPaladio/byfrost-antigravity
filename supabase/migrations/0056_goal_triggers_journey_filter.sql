-- Migration 0056: Goal Triggers Journey Filter

-- Add filter_journey_id column
alter table public.goal_triggers 
add column if not exists filter_journey_id uuid references public.journeys(id) on delete cascade;

-- Replace unique constraint to allow multiple journey-specific triggers
drop index if exists public.goal_triggers_tenant_event_metric_idx;

create unique index if not exists goal_triggers_unique_idx 
on public.goal_triggers(
    tenant_id, 
    event_type, 
    metric_key, 
    coalesce(filter_journey_id, '00000000-0000-0000-0000-000000000000'::uuid)
);

-- Update Trigger Function to respect the filter
